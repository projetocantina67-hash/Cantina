    ...
    return `
        <div class="container" style="max-width:1400px; padding-top:20px; padding-bottom:40px;">
            ...
            <!-- SELETOR DE ABA (KANBAN VS HISTÓRICO) -->
                ...
                <button onclick="window.app.switchAdminTab('metricas')" style="padding:10px 20px; border-radius:10px; font-weight:700; border:none; cursor:pointer; ${activeTab === 'metricas' ? 'background:var(--primary); color:white;' : 'background:#e5e7eb; color:#4b5563;'}">
        ...
        : activeTab === 'historico' ? renderHistoricoView()
            : activeTab === 'produtos' ? renderProdutosView()
                : activeTab === 'funcionarios' ? renderFuncionariosView()
                    : renderMetricasView()}
...
function renderKanbanCard(order) {
    const time = new Date(order.horarioEstimadoRetirada).toLocaleTimeString('pt-BR', {
        hour: '2-digit',
        minute: '2-digit'
    });
    ...
    return `
        <div style="background:white; border-radius:12px; padding:16px; border:1px solid #e5e7eb; box-shadow:0 2px 8px rgba(0,0,0,0.04);">
            ...
            <div style="margin-bottom:12px; padding:10px; background:#f9fafb; border-radius:8px; font-size:13px; color:#374151;">
                ...
                <span><strong>${i.quantidade}x</strong> ${i.nomeProduto}</span>                 
    ...
    return `
        <div style="padding:12px; border:1px solid #f3f4f6; border-radius:8px; display:flex; justify-content:space-between; align-items:center; font-size:13px;">
            <div>
                ...
                <div style="color:#6b7280; font-size:12px;">${(order.itens || []).map(i => `${i.quantidade}x ${i.nomeProduto}`).join(', ')}</div>
            </div>
...
function renderMetricasView() {
    const todosPedidos = [
        ...currentOrders.PENDENTE,
        ...currentOrders.EM_PREPARO,
        ...currentOrders.PRONTO,
        ...currentOrders.ENTREGUE,
        ...currentOrders.CANCELADO
    ];

    const pedidosValidos = todosPedidos.filter(p => p.status !== 'CANCELADO');
    const pedidosCancelados = currentOrders.CANCELADO;

    const receitaBruta = pedidosValidos.reduce((sum, p) => sum + (p.valorTotal || 0), 0);
    const receitaRecebida = pedidosValidos.filter(p => p.pago).reduce((sum, p) => sum + (p.valorTotal || 0), 0);
    const receitaPendente = receitaBruta - receitaRecebida;
    const receitaPerdida = pedidosCancelados.reduce((sum, p) => sum + (p.valorTotal || 0), 0);

    const totalItensVendidos = pedidosValidos.reduce((sum, p) =>
        sum + (p.itens || []).reduce((s, i) => s + (i.quantidade || 0), 0), 0);

    const ticketMedio = pedidosValidos.length > 0 ? receitaBruta / pedidosValidos.length : 0;

    const rankingMap = {};
    pedidosValidos.forEach(p => {
        (p.itens || []).forEach(i => {
            const nome = i.nomeProduto || 'Produto';
            if (!rankingMap[nome]) rankingMap[nome] = { quantidade: 0, receita: 0 };
            rankingMap[nome].quantidade += i.quantidade || 0;
            rankingMap[nome].receita += (i.subtotal ?? (i.precoUnitario * i.quantidade)) || 0;
        });
    });
    ...
    const statusCounts = [
        { label: 'Pendentes', valor: currentOrders.PENDENTE.length, cor: '#92400e', bg: '#fef3c7' },
        { label: 'Em Preparo', valor: currentOrders.EM_PREPARO.length, cor: '#9a3412', bg: '#ffedd5' },
        { label: 'Prontos', valor: currentOrders.PRONTO.length, cor: '#166534', bg: '#dcfce7' },
        { label: 'Entregues', valor: currentOrders.ENTREGUE.length, cor: '#4b5563', bg: '#f3f4f6' },
        { label: 'Cancelados', valor: currentOrders.CANCELADO.length, cor: '#991b1b', bg: '#fee2e2' },
    ];

    const metricCard = (icon, label, value, color) => `
        <div style="background:white; border-radius:16px; padding:20px; border:1px solid #e5e7eb; flex:1; min-width:200px;">
            <div style="display:flex; align-items:center; gap:10px; color:#6b7280; font-size:13px; font-weight:700; text-transform:uppercase; margin-bottom:10px;">
                <i class="fa-solid ${icon}" style="color:${color};"></i> ${label}
            </div>
            <div style="font-size:26px; font-weight:900; color:#111827;">${value}</div>
        </div>
    `;

    return `
        <div style="display:flex; flex-wrap:wrap; gap:15px; margin-bottom:20px;">
            ${metricCard('fa-sack-dollar', 'Receita Bruta', `R$ ${formatPrice(receitaBruta)}`, '#16a34a')}
            ${metricCard('fa-circle-check', 'Receita Recebida (Pago)', `R$ ${formatPrice(receitaRecebida)}`, '#0369a1')}
            ${metricCard('fa-hourglass-half', 'A Receber (Pendente)', `R$ ${formatPrice(receitaPendente)}`, '#d97706')}
            ${metricCard('fa-boxes-packing', 'Itens Vendidos', totalItensVendidos, '#6d28d9')}
            ${metricCard('fa-receipt', 'Ticket Médio', `R$ ${formatPrice(ticketMedio)}`, '#111827')}
            ${metricCard('fa-ban', 'Receita Perdida (Cancelados)', `R$ ${formatPrice(receitaPerdida)}`, '#dc2626')}
        </div>
        ...
        <div style="display:grid; grid-template-columns: 1.2fr 1fr; gap:20px; align-items:start;">
            <div style="background:white; border-radius:16px; padding:20px; border:1px solid #e5e7eb;">
                <h3 style="margin-top:0; color:#111827; display:flex; align-items:center; gap:8px;">
                    <i class="fa-solid fa-ranking-star" style="color:#d97706;"></i> Produtos Mais Vendidos
                </h3>
                ${ranking.length > 0 ? `
                    <div style="display:flex; flex-direction:column; gap:8px;">
                        ${ranking.map((r, idx) => `
                            <div style="display:flex; justify-content:space-between; align-items:center; padding:10px 12px; background:#f9fafb; border-radius:8px;">
                                <div style="display:flex; align-items:center; gap:10px;">
                                    <span style="font-weight:800; color:#9ca3af; width:20px;">#${idx + 1}</span>
                                    <span style="font-weight:600; color:#111827;">${r.nome}</span>
                                </div>
                                <div style="text-align:right;">
                                    <div style="font-weight:800; color:#111827;">${r.quantidade}x</div>
                                    <div style="font-size:12px; color:#6b7280;">R$ ${formatPrice(r.receita)}</div>
                                </div>
                            </div>
                        `).join('')}
                    </div>
                ` : '<p style="color:#9ca3af;">Nenhum item vendido ainda.</p>'}
            </div>

            <div style="background:white; border-radius:16px; padding:20px; border:1px solid #e5e7eb;">
                <h3 style="margin-top:0; color:#111827; display:flex; align-items:center; gap:8px;">
                    <i class="fa-solid fa-layer-group"></i> Pedidos por Status
                </h3>
                <div style="display:flex; flex-direction:column; gap:8px;">
                    ${statusCounts.map(s => `
                        <div style="display:flex; justify-content:space-between; align-items:center; padding:10px 12px; background:${s.bg}; border-radius:8px;">
                            <span style="font-weight:700; color:${s.cor};">${s.label}</span>
                            <span style="font-weight:900; color:${s.cor};">${s.valor}</span>
                        </div>
                    `).join('')}
                </div>
                <p style="margin-top:15px; font-size:12px; color:#9ca3af;">
                    * Métricas calculadas com base nos pedidos atualmente carregados no painel (até 500 por status).
                </p>
            </div>
        </div>
    `;
...
export async function desativarUsuarioAdmin(id) {
    ...
    try {
        ...
        window.app.showToast('Erro ao desativar acesso.', 'error');
    }

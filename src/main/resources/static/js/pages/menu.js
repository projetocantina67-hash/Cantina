...
export function renderMenu(selectedCategory = 'Todos') {
    ...
    const normalizedSelectedCategory = String(selectedCategory ?? 'Todos').trim();
    const filteredProducts = normalizedSelectedCategory === 'Todos'
        ? products
        : products.filter(p => {
            const categoryName = String(p.category ?? '');
            return categoryName.toLowerCase().replace(/_/g, '').replace(/\s+/g, '') === normalizedSelectedCategory.toLowerCase().replace(/_/g, '').replace(/\s+/g, '');
        });
